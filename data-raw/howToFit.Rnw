...
opt_result <- optim(par = param_vector,
      fn = loglik_math,
      method = "BFGS",
      env_dat = env_array,
      occ = occ,
...
{
  dists_to_first[counter] <- dist_between_params(
...
starts_1 <- start_parms(env_array[occ == 1, ,1, drop = FALSE],
                        num_starts = num_starts)
starts_2 <- start_parms(env_array[occ == 1, ,2, drop = FALSE],
                        num_starts = num_starts)
...
convergence_1 <- sapply(X = all_optim_results_1,
                         FUN = function(x){x$convergence})
table(convergence_1)
bestlogliks_1 <- sapply(X = all_optim_results_1,
...
convergence_2 <- sapply(X = all_optim_results_2,
                        FUN = function(x){x$convergence})
table(convergence_2)
bestlogliks_2 <- sapply(X = all_optim_results_2,
                        FUN = function(x){x$value})
...
for (counter in 1:num_starts)
{
  dists_to_first_2[counter] <- dist_between_params(
    all_optim_results_2[[1]]$par,
...
<<rectify>>=
true_parameters_bio <- example_1$par_list
ML_parameters_bio = math_to_bio(all_optim_results[[1]]$par)
example_1_true_parameters_bio <- dist_between_params(
  true_parameters_bio,
  ML_parameters_bio,
...
plot_tool <- function(x, y, thresh, xlab, true_param) {
  ...
  lines(range(x), rep(thresh, 2), type = "l", lty = "dashed", col = "red")
  points(true_param, thresh, col = "red", pch = 20, cex = 2)
}
