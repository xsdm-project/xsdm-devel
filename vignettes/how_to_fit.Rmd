...
vignette: >
  %\VignetteIndexEntry{How to fit xsdm models with species occurrence data using xsdm}
...
**Abstract.** After reading the document entitled "The xsdm model", this document
introduces the statistically and computationally sophisticated user to the 
main functions of the `xsdm` package. The package implements a frequentist 
...
bio_1 <- terra::unwrap(example_1_bio01) 
class(bio_1)
dim(bio_1)
...
```{r bio12}
bio_12 <- terra::unwrap(example_1_bio12) 
class(bio_12)
dim(bio_12)
...
```{r load_species_data}
d <- readRDS("DetectionsNonDetections.Rds")
class(d)
dim(d)
head(d)
sum(d$presence==1)
...
```{r look_at_data_03}
pts_0 <- terra::vect(d[d$presence==0,],geom=c("x","y"),crs=terra::crs(m_bio_1))
pts_1 <- terra::vect(d[d$presence==1,],geom=c("x","y"),crs=terra::crs(m_bio_1))
par(mfrow=c(1,2))
...
The likelihood is coded as `loglik\_bio`, so as an introduction to the function 
...
`sigltil`, `sigrtil`, `pd`, and `o\_mat`; see previous section). 
...
in `xsdm` using the function `math\_to\_bio`, which takes an unconstrained
numeric vector as its argument and returns a named list of of biological-scale
parameters. But it is not so common for the end-user to call `math\_to\_bio`
directly because we have written the likelihood directly in terms of the math-scale
parameters in a function `loglik\_math`. We now demonstrate `math\_to\_bio`
and `loglik\_math`:
```{r loglik_bio_eval3}
...
biological scale using `math\_to\_bio` and
then evaluates `loglik\_bio`; so one optimizes it directly 
...
Before moving on to optimizing, we note that `loglik\_math`
requires a named vector for its input `param\_vector`. This is to reduce
...
in the documentation for `loglik\_math`. 
The function `make\_mask\_names` helps the user by giving the parameter names which
...
The function `start\_parms` can be used to find plausible initial conditions
for optimization:
```{r optimizer_starts}
num_starts <- 10
...
environmental variables or if warranted after running 50 (see below). But for this exercise we use only `r num_starts` 
...
These results indicate that: 1) most of the `r num_starts` optimizations appear to have
converged according to the diagnostics of `optim` (0 means convergence for 
`optim`); and 2) five of the `r num_starts` optimizations returned the same, highest 
...
These are in the same order as the maximized likelihood values above. Note that the first five
...
each other with respect to the `o\_par1` parameter. And the `sigltil1`,
...
`math\_to\_bio` mapping is many-to-one, and that there are also multiple ways to 
parameterize the identical xsdm model with distinct biological-scale parameters. These redundancies
affect the `sigltil1`,
`sigltil2`, `sigrtil1`, `sigrtil2`, and `o\_par1` parameters.
For models making use of more than two environmental variables, all the `o\_par` 
...
with parameter redundancy," but the function `distance\_between\_params\_hungarian`
...
described; i.e., if `distance\_between\_params\_hungarian` indicates a very small
...
for (counter in 1:num_starts)
{
  dists_to_first[counter] <- 
    distance_between_params_hungarian(
...
Note that the first five parameter optimization results  are all reported to be close in 
...
`distance\_between\_params\_hungarian` to be close to each other in parameter space, we
...
for (counter in 1:num_starts)
{
  dists_to_first_1[counter] <- 
    distance_between_params_hungarian(all_optim_results_1[[1]]$par,
...
for (counter in 1:num_starts)
{
  dists_to_first_2[counter] <- 
    distance_between_params_hungarian(all_optim_results_2[[1]]$par,
...
`profile\_likelihood`, here for \text{mu1}:
...
when `num\_steps\_left` (resp., `num\_steps\_right`) is exceeded, or
...
`increment\_left`, `increment\_right`, `num\_steps\_left`,
and `num\_steps\_right` to get complete and smooth profiles within the 
...
`parameters` output of `profile\_likelihood`:
...
Finally, the `found\_better` output of `profile\_likelihood` is
...
If `found\_better` is `TRUE`, it means you have to go back and 
...
example_1_true_parameters_bio<-
  distance_between_params_hungarian(example_1_true_parameters_bio,
                                    ML_parameters_bio,
...
```{r plot_all_profiles, fig.width=6}
par(mfrow=c(3,3))
...
Note that transforming `o\_par` profiles to the biological scale
...
the `vsp` function. The `vsp` function has the dual purposes of 
...
par(mfrow=c(2,3))
hab_suit <- vsp(env_data,ML_parameters_bio,return_raster=TRUE)
terra::plot(hab_suit,main="Habitat suitability",xlab="x",ylab="y",legend=TRUE)

#plot the true habitat suitability map
hab_suit_true <- vsp(env_data,example_1_true_parameters_bio,return_raster=TRUE)
...
m_hab_suit <- vsp(m_env_data,ML_parameters_bio,return_raster=TRUE)
...
So we see variability reduces area by `r round(100*delta_area,2)`%
...
for details). The function `interpret\_parameters` helps with this, displaying
...
```{r interpret_parameters, fig.height=4}
devtools::load_all()
...
```{r interpret_parameters_true, fig.height=4}
par(mfrow=c(1,2))
...
of `interpret\_parameters` for details.
