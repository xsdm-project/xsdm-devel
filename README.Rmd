...
knitr::opts_chunk$set(
  ...
  out.width = "100%",
  eval      = FALSE   # most chunks are illustrative; knit with eval=TRUE when releasing
)
```

# xsdm <img src="man/figures/README-pressure-1.png" align="right" height="120" alt="" />
...
- Reconstruct a species' **fundamental ecological niche** via maximum-likelihood
  estimation.
- Account for **inter-annual climate variability** when estimating niche breadth
  and position.
- Project the species' **potential geographic range** under current or future
  climate scenarios.

The statistical underpinning is described in:

> Berti, E., Robles Fernández, A.L., Rosenbaum, B., Peterson, T.A., Soberón, J.,
> & Reuman, D.C. (2025). *The impacts of climate variability on the niche
> concept and distributions of species*. bioRxiv.
...
xsdm requires **R ≥ 4.1** and links to C++ via Rcpp/RcppParallel, so a working
compiler (e.g. Rtools on Windows, Xcode CLT on macOS) is needed.

---

## Quick start

The package ships with a built-in dataset (`examples`) containing all objects
needed to run a complete workflow without loading external files.
...
[CHELSA v2.1](https://www.chelsa-climate.org/) bio1 (mean annual temperature)
and bio12 (annual precipitation).
...
`env_data_array()` extracts and stacks environmental values at the locations
given in a presence/absence data frame, returning a 3-D array
...
Convert the best math-scale parameter vector to the biologically interpretable
scale and plot the inferred log growth–environment function:
...
`vsp()` uses the fitted parameter list and the full-grid rasters to return
...
`profile_likelihood()` fixes a target parameter at a grid of values, re-optimises
all others at each grid point, and returns the profile log-likelihood together
with a likelihood-ratio confidence threshold.
...
Tests are written with **testthat** (≥ 3.0) and cover all exported functions.
Code coverage is tracked via [Codecov](https://app.codecov.io/gh/xsdm-project/xsdm-devel).
...
[GitHub issue tracker](https://github.com/xsdm-project/xsdm-devel/issues).
When reporting a bug, include the output of `sessionInfo()` and a minimal
reproducible example.

### Extending xsdm

The internal log-likelihood engine is implemented in C++ via **Rcpp** and
**RcppParallel**.  For statistically or computationally sophisticated users,
the raw likelihood function (`loglik_math()`) and parameter-transformation
utilities (`math_to_bio()`, `bio_to_math()`, `make_mask_names()`) are fully
exported, enabling custom optimization workflows.
...
xsdm is released under the [GNU General Public License v3 or later](LICENSE.md).
