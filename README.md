...
The package ships with a built-in dataset (`examples`) containing all
...
# The examples dataset is a named list bundling all example objects.
names(examples)
#> [1] "par_vec"                  "bio01"
#> [3] "bio12"                    "env_array"
#> [5] "occ_df"                   "occ_vec"
...
# examples$occ_df has columns: name, x (lon), y (lat), presence (0/1)
env_dat <- env_data_array(env_data, occ = examples$occ_df)
dim(env_dat)
#> [1] 1000   39    2    (locations × time-steps × variables)

occ <- examples$occ_df$presence
```
...
result <- optimize_likelihood(
  env_dat    = env_dat,
  occ        = occ,
  num_starts = 100L,       # increase for a real analysis
  parallel   = FALSE,      # set TRUE to use future/furrr parallelism
  verbose    = TRUE
)
