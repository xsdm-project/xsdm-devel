...
#' )
#'
#' # p2 is equivalent to p1: columns swapped and one sign flipped
#' p2 <- p1
#' p2$o_mat <- matrix(c(0, -1, 1, 0), 2, 2)  # swapped columns, second column negated
#' p2$sigltil <- p1$sigltil[c(2, 1)]
#' p2$sigrtil <- p1$sigrtil[c(2, 1)]
#' p2$sigltil[2] <- p1$sigrtil[1]  # because of sign flip, sigltil and sigrtil swap
...
distance_between_params <- function(p1, p2, GiveClosestRep = FALSE) {
  ...
  dd <- dim(p1$o_mat)[1]
  if (dd > 5) {
    warning("In distance_between_params: this function is very slow for large numbers of environmental variables")
  }
  
  # compute square distance for all parameters except o_mat, sigltil, and sigrtil
  sq_dist_other_params <- sum((p1$mu - p2$mu)^2) + (p1$ctil - p2$ctil)^2 + (p1$pd - p2$pd)^2
  
  # get lists of all the sign flip patterns and all the permutations which are to
  ...
  # now iterate through representatives of the equivalence class of p1 and compute
  # distances to p2
  bestsqdist <- Inf
  bestparams <- NA
  for (fc in 1:(dim(all_flips)[1]))
  {
    for (pc in 1:(dim(all_perms)[1]))
    {
      ...
      thissqdist <- sum((p1_ec$o_mat - p2$o_mat)^2) + sum((1 / p1_ec$sigltil - 1 / p2$sigltil)^2) + sum((1 / p1_ec$sigrtil - 1 / p2$sigrtil)^2)
