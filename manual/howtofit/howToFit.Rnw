...
% --- Header material ---
\title{How to fit xsdm models with species occurrence data using \texttt{xsdmMle}}
...
main functions of the \texttt{xsdmMle} package. The package implements a frequentist 
...
This manual uses a small built-in example included with the \texttt{xsdmMle} R 
...
The fitting tools offered by \texttt{xsdmMle} use a more succinct version of the data,
...
in \texttt{xsdmMle} using the function \texttt{math\_to\_bio}, which takes an unconstrained
...
environmental variables or if warranted after running 50 (see below). But for this exercise we use only \Sexpr{num_starts} 
to keep run times low. 
We now optimize from all the start parameters:
<<optimizing_many, cache=TRUE>>=
...
bestlogliks
@
\noindent These results indicate that: 1) most of the \Sexpr{num_starts} optimizations appear to have
converged according to the diagnostics of \texttt{optim} (0 means convergence for 
\texttt{optim}); and 2) five of the \Sexpr{num_starts} optimizations returned the same, highest 
...
<<fitting_simpler_models, cache=TRUE>>=
...
To calculate profiles using \texttt{xsdmMle}, use the function 
\texttt{profile\_likelihood}, here for \text{mu1}:
<<profiling1, cache=TRUE>>=
...
<<plot_profile, fig.width=4, fig.asp=1, fig.pos='H'>>=
...
<<parameters_profile_output, fig.pos='H'>>=
...
<<plot_all_profiles, fig.width=6, fig.pos='H'>>=
par(mfrow=c(3,3))
...
One of the main things \texttt{xsdmMle} is good for is assessing 
...
<<habitat_suitability_novar, fig.pos='H'>>=
...
\noindent So we see variability reduces area by \Sexpr{round(100*delta_area,2)}\%
...
<<interpret_parameters, fig.pos='H', fig.height=4>>=
devtools::load_all()
...
<<interpret_parameters_true, fig.pos='H', fig.height=4>>=
par(mfrow=c(1,2))
